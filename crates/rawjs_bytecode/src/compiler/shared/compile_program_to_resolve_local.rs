...
impl Compiler {
    fn compile_program_with_mode(program: &Program, return_last_expression: bool) -> Result<Chunk> {
        ...
        let repl_result_index = if return_last_expression {
            match program.body.last() {
                Some(Statement::Expression(_)) => Some(program.body.len() - 1),
                _ => None,
            }
        } else {
            None
        ...
        for (index, stmt) in program.body.iter().enumerate() {
            ...
            if repl_result_index == Some(index) {
                if let Statement::Expression(es) = stmt {
                    compiler.compile_expression(&es.expression)?;
...
impl Compiler {
    pub(super) fn declare_local_with_storage(
        &mut self,
        name: &str,
