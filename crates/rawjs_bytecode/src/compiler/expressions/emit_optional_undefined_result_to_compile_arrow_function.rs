impl Compiler {
    pub(super) fn emit_optional_undefined_result(&mut self) {
...
impl Compiler {
    pub(super) fn emit_optional_method_undefined_result(&mut self) {
        self.emit(Instruction::Pop);
        self.emit(Instruction::Pop);
        self.emit(Instruction::Undefined);
    }
...
impl Compiler {
    pub(super) fn compile_member_access(&mut self, member: &MemberExpression) -> Result<()> {
        if member.computed {
...
impl Compiler {
    pub(super) fn compile_array(&mut self, arr: &ArrayExpression) -> Result<()> {
...
impl Compiler {
    pub(super) fn compile_object(&mut self, obj: &ObjectExpression) -> Result<()> {
...
impl Compiler {
    pub(super) fn compile_function_expression(&mut self, func: &FunctionDeclaration) -> Result<()> {
        self.compile_function_body(func)
    }
}

impl Compiler {
    pub(super) fn compile_arrow_function(&mut self, arrow: &ArrowFunctionExpression) -> Result<()> {
        let name = "<arrow>";
