impl Compiler {
    pub(crate) fn compile_function_body(&mut self, func: &FunctionDeclaration) -> Result<()> {
        ...
        child.is_strict =
            self.is_strict || crate::compiler::has_use_strict_directive(&func.body.body);
...
impl Compiler {
    pub(super) fn compile_yield(&mut self, yield_expr: &YieldExpression) -> Result<()> {
...
impl Compiler {
    pub(super) fn compile_await(&mut self, await_expr: &AwaitExpression) -> Result<()> {
        if !self.is_async {
            return Err(RawJsError::syntax_error(
