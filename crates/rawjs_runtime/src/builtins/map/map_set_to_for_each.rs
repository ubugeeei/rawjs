pub(super) fn map_set(_heap: &mut Heap, this: &JsValue, args: &[JsValue]) -> Result<JsValue> {
...
pub(super) fn map_has(_heap: &mut Heap, this: &JsValue, args: &[JsValue]) -> Result<JsValue> {
...
pub(super) fn map_delete(_heap: &mut Heap, this: &JsValue, args: &[JsValue]) -> Result<JsValue> {
    let key = args.first().unwrap_or(&JsValue::Undefined);
    match this {
        JsValue::Object(ptr) => {
            let removed = {
...
pub(super) fn map_clear(_heap: &mut Heap, this: &JsValue, _args: &[JsValue]) -> Result<JsValue> {
    match this {
        JsValue::Object(ptr) => {
            {
                let mut obj = ptr.borrow_mut();
