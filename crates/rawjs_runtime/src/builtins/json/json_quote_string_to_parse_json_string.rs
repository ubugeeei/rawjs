...
pub(super) fn json_parse(heap: &mut Heap, _this: &JsValue, args: &[JsValue]) -> Result<JsValue> {
    let text = args
        .first()
...
pub(super) fn parse_json_value<'a>(heap: &mut Heap, input: &'a str) -> Option<(JsValue, &'a str)> {
...
pub(super) fn parse_json_string(input: &str) -> Option<(String, &str)> {
