...
pub(super) fn async_generator_return(
    vm: &mut Vm,
    gen_ptr: &rawjs_runtime::GcPtr<JsObject>,
    value: JsValue,
) -> JsValue {
...
pub(super) fn async_generator_throw(
    vm: &mut Vm,
    gen_ptr: &rawjs_runtime::GcPtr<JsObject>,
    error: JsValue,
) -> JsValue {
...
pub(super) fn make_iterator_result(vm: &mut Vm, value: JsValue, done: bool) -> JsValue {
...
pub(super) enum DisposeResult {
    Complete,
    Await(JsValue),
}

pub(super) fn create_async_result_promise(vm: &mut Vm) -> GcPtr<JsObject> {
