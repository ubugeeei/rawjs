pub(super) fn jump_relative(vm: &mut Vm, offset: i32) {
    let frame = vm.call_stack.last_mut().unwrap();
    frame.ip = ((frame.ip as i64) + (offset as i64)) as usize;
}

pub(super) fn jump_if(vm: &mut Vm, offset: i32, expected: bool) -> Result<()> {
    let value = vm.pop()?;
...
pub(super) fn jump_if_nullish(vm: &mut Vm, offset: i32) -> Result<()> {
...
pub(super) fn execute_call_method(vm: &mut Vm, argc: usize) -> Result<()> {
...
pub(super) fn create_ordinary_object(vm: &mut Vm) -> GcPtr<JsObject> {
...
pub(super) fn create_array_object(vm: &mut Vm, count: u16) -> Result<GcPtr<JsObject>> {
...
pub(super) fn mirror_global_property(vm: &mut Vm, obj_val: &JsValue, key: &str, value: &JsValue) {
...
pub(super) fn get_computed_value(vm: &mut Vm, obj_val: JsValue, key: JsValue) -> Result<JsValue> {
...
pub(super) fn set_computed_value(
    vm: &mut Vm,
    obj_val: JsValue,
    key: JsValue,
    value: JsValue,
